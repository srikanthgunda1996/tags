    ...
    public class Command2 : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            t.Start("Creating Tags");
            {
                ...
                FilteredElementCollector views = new FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_Views).WhereElementIsNotElementType();
                int totaltagcount = 0;
                foreach(View curview in views)
                {
                    curview.DetailLevel = ViewDetailLevel.Fine;
                    //curview.SetViewDisplayModel = ViewDetailLevel.Fine;
                    int count = 0;
                    if(curview.ViewType == ViewType.Rendering) { TaskDialog.Show("View", "Rendering View"); }
                    if(curview.ViewType == ViewType.ThreeD) { TaskDialog.Show("View", "3d View"); }
                    if (curview.ViewType == ViewType.Elevation || curview.ViewType == ViewType.Schedule) { TaskDialog.Show("View", "Elevation View"); }
                    if(curview.IsTemplate == true) { continue; }
                ...
                TaskDialog.Show("Total Tags", $" Total {totaltagcount} tags were placed");
            }
